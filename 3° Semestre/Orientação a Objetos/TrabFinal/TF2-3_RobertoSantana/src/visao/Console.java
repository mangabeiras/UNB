...
public class Console {
	...
	// RESULTADO
	public static void mostraResultado(List<Pessoa> grupo) {
		...
		System.err.println(mascara.format(grupo.size()) + " = TOTAL DE REGISTRO DE PESSOAS");
	}

	// QUANTIDADE DE MULHERES OU HOMENS SEM CONTAMINACAO
	private static int pessoasSemContaminacao(List<Pessoa> grupo, char a) {
		int casos = 0;
		for (Pessoa pessoa : grupo) {
	...
	private static int calculaCasos(List<Pessoa> grupo, char a) {
		int casos = 0;
		for (Pessoa pessoa : grupo) {
