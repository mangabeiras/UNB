...
public class Lista extends JFrame {
	...
	private void criaPainelTabela() {
		...
		painelTabela.setBounds(0, 0, 784, 380);
	...
	private void preencheTabela() {
		...
		for (Pessoa p : dao.getGrupo().getGrupo()) {
			if (p.toString().split(" ") [0].equals("--"))
				modelo.addRow(new Object[] {p.getId(), p.getNome().toLowerCase(), p.getSituacaoExtenso(), "--",((Homem) p).getIdade()});
			else
				modelo.addRow(new Object[] {p.getId(), p.getNome().toLowerCase(), p.getSituacaoExtenso(),((Mulher) p).getSituacaoGExtenso(), "--"});
		}
	}
